...
class DomainRandomizationMixin:
    def __init__(
        ...
        self.HOVER_RPM = float(np.sqrt((self.G * self.M) / (4.0 * self.KF)))
    ...
    def set_dr_params(
        ...
        if update_log:
            self._dr_last = {"mass": self.M, "kf": self.KF, "km": self.KM, "hover_rpm": self.HOVER_RPM}
    ...
    def _dr_sample_scales(self):
        sample_mass = self._dr_rng.uniform(*self._dr_mass_range)
        sample_kf = self._dr_rng.uniform(*self._dr_kf_range)
        sample_km = self._dr_rng.uniform(*self._dr_km_range)
        return sample_mass, sample_kf, sample_km
    ...
    def _apply_domain_randomization(self):
        sm, skf, skm = self._dr_sample_scales()
